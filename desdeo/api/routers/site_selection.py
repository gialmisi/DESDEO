...
from desdeo.api.models import ProblemDB, ProblemMetaDataDB, SiteSelectionMetaData, User, UserRole
...
def load_metadata(
    ...
    # Role hierarchy: admin > analyst > dm > guest.
    # Users can manage metadata for their own problems or problems owned by lower-role users.
    _role_rank = {UserRole.guest: 0, UserRole.dm: 1, UserRole.analyst: 2, UserRole.admin: 3}
    is_own = problem.user_id == user.id
    outranks_owner = _role_rank.get(user.role, 0) > _role_rank.get(problem.user.role, 0)
