...
_coco_locator_evaluate = "desdeo://external/coco/evaluate"


class CocoProblemParams(ExternalProblemParams):
    """Parameters to generate coco test problems.
...
def _params_key(params: dict) -> str:
    """Canonical cache key for a CocoProblemParams instance.

    Canonical cache key for a CocoProblem instance.
...
def _get_cached_pymoo_problem(params_key: str):
    """Cache coco Problem instances by a stable JSON key.
...
def _get_cached_info(params_key: str) -> ExternalProblemInfo:
    """Cache ExternalProblemInfo too, since it derives from the cached coco problem."""
    """
...
class CocoProvider(Provider):
    """Provider to get info on and evaluate coco test problems.

    Note:
        the methods `info` and `evaluate` use caching so that the coco problem is generated only once
            in case of multiple consecutive evaluations of the same problem.
    """

    def info(self, params: CocoProblemParams | dict[str, Any]) -> ExternalProblemInfo:
        """Get info on a coco test problem.

        Args:
            params (CocoProblemParams | dict[str, Any]): parameters to generate the problem.
    ...
    def evaluate(
        self, xs: dict[str, VariableType | list[VariableType]], params: CocoProblemParams | dict[str, Any]
    ) -> dict[str, float]:
        """Evaluate a coco test problem.

        Args:
            xs (dict[str, VariableType]): a set of variables to be evaluated.
                Expected format is {variable symbol: [values]}.
            params (ProviderParams | dict[str, Any]): parameters that generate the problem coco
...
def create_pymoo_problem(params: CocoProblemParams) -> Problem:
    """Create a coco test problem based on given parameters.

    Args:
        params (PymooProblemParams): the parameters to generate the coco test problem.

    Returns:
        Problem: an instance of a coco test problem generated based on the given parameters.
    ...
    Note:
        ...
        Ideal and nadir point values will be available, if they are available in coco for a given problem.

        For info on the possible problems to generate, see
    """
    provider = CocoProvider()
    info = provider.info(params)

    simulator_url = Url(url=_coco_locator_evaluate)
