...
Values significantly better than Baseline (paired Wilcoxon, α=0.025 after
...
def _wilcoxon_test(
    ...
    # One-sided: test if alt is better (d > 0)
    try:
        stat, p_value = wilcoxon(d_nonzero, alternative="greater")
    except ValueError:
        return {
            "W": None,
            "p_value": None,
...
def snakemake_main() -> None:  # noqa: D103
        ...
        for ps in population_sizes:
            for ct in ct_levels:
                ...
                for alt_mode in alt_modes:
                    ...
                    # Mark as significant only if alt is significantly better
    ...
    lines.append(
        ...
        + " by mode ("
        + abbrev_legend
        + ").  \\textbf{Bold} = significantly better than Baseline"
        + f" (Wilcoxon, $\\alpha={ALPHA}$, Bonferroni-corrected)."
        ...
        for row_in_block, ps in enumerate(population_sizes):
            ...
            for ct in ct_levels:
                ...
                for m in modes:
                    ...
                    # Bold if this mode is significantly better than baseline
                    if sig_results.get((prob, ps, ct, m), False):
                        s = f"\\textbf{{{s}}}"
                    data_cells.append(s)
